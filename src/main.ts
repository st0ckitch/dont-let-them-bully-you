...
import { MOVES } from './moves/index.ts';
import type { Move } from './moves/types.ts';
import { loadAssets } from './anim.ts';
import { Fighter3D } from './fighter3d.ts';
import { Engine, simFight, setFightRng, seededRng } from './modes/octagon/fight.ts';
import type { EngineCallbacks, FighterActor } from './modes/octagon/fight.ts';
import { Net, PROTOCOL_VERSION } from './net.ts';
import type { NetCallbacks } from './net.ts';
import { AutochessMode, PHASE as AUTOCHESS_PHASE } from './modes/autochess/mode.ts';
import { AutochessUI } from './modes/autochess/ui.ts';
import { cellId as autochessCellId } from './modes/autochess/hex.ts';
...
function reportFatal(msg: string): void {
...
interface Voices { [id: string]: HTMLAudioElement }
...
class FX {
  ...
  voices?: Voices;
  announces?: Voices;
  private _elementsUnlocked = false;
  private _voiceUntil = 0;
  musicOn = false;
  mGain?: GainNode;
  private mPad: (OscillatorNode)[] = [];
  private mBar = 0;
  private mNext = 0;
  private mTimer?: ReturnType<typeof setInterval>;
  ...
  impact(kind: 'punch' | 'kick', heavy: boolean, crit: boolean): void {
    ...
    const vary = (f: number): number => f * (0.88 + Math.random() * 0.24);
  ...
  startMusic(): void {
    ...
    this.mPad = [padLfo];
    const padVoices: [number, number][] = [[146.83, -5], [146.83, 5], [220, 3]];
    for (const [f, det] of padVoices) {
      const o = c.createOscillator();
      o.type = 'sawtooth';
      o.frequency.value = f;
      o.detune.value = det;
    ...
    this.mTimer = setInterval(() => {
      while (this.musicOn && this.ctx && this.mNext < this.ctx.currentTime + 0.45) {
  ...
  duckMusic(): void {
    if (!this.musicOn || !this.ctx || !this.mGain) return;
    const t = this.ctx.currentTime;
    const g = this.mGain.gain;
    g.cancelScheduledValues(t);
    g.setValueAtTime(g.value, t);
    g.linearRampToValueAtTime(MUSIC_VOL * 0.3, t + 0.05);
    g.setTargetAtTime(MUSIC_VOL, t + 0.4, 0.3);
  }

  private _musicBar(t0: number, bar: number): void {
    const step = MUSIC_BAR / 16;
    const hit = (fn: (t: number, ...rest: number[]) => void, s: number, ...args: number[]): void =>
      fn.call(this, t0 + s * step, ...args);
  ...
  private _mKick(t: number): void {
    const c = this.ctx;
    if (!c || !this.mGain) return;
    const o = c.createOscillator();
  ...
  private _mSnare(t: number, vol: number): void {
    const c = this.ctx;
    if (!c || !this.mGain) return;
    const src = this._noise(c, 0.12);
    const bp = c.createBiquadFilter();
    bp.type = 'bandpass';
  ...
  private _mHat(t: number, vol: number): void {
    const c = this.ctx;
    if (!c || !this.mGain) return;
    const src = this._noise(c, 0.035);
  ...
  private _mBass(t: number, freq: number): void {
    const c = this.ctx;
    if (!c || !this.mGain) return;
    const o = c.createOscillator();
    o.type = 'sawtooth';
  ...
  private _mTom(t: number, freq: number): void {
    const c = this.ctx;
    if (!c || !this.mGain) return;
    const o = c.createOscillator();
  ...
  private _noise(c: AudioContext, sec: number): AudioBufferSourceNode {
  ...
  blockHit(heavy: boolean): void {
    ...
    const thump = this._noise(c, 0.05);
    const lp = c.createBiquadFilter();
    lp.type = 'lowpass';
    ...
    const clack = this._noise(c, 0.02);
...
const fx = new FX();
(window as unknown as { __fx: FX }).__fx = fx;
...
const STANCE_LABEL: Record<string, string> = { orthodox: 'Orthodox', southpaw: 'Southpaw' };

function setPlate(side: 'a' | 'b', cfg: Fighter): void {
  const card = $(side === 'a' ? '#card-a' : '#card-b');
  card.querySelector<HTMLElement>('.flag')!.textContent = cfg.flag;
  const [first, ...rest] = cfg.name.split(' ');
  card.querySelector<HTMLElement>('.pfirst')!.textContent = first.toUpperCase();
  card.querySelector<HTMLElement>('.plast')!.textContent = rest.join(' ').toUpperCase();
  card.querySelector<HTMLElement>('.nick')!.textContent = cfg.nick.toUpperCase();
  card.querySelector<HTMLElement>('.hpwrap')!.setAttribute('aria-label', `${cfg.name} health`);
  const rows = [
  ...
  card.querySelector<HTMLElement>('.skills')!.innerHTML = rows
    .map(([n, v]) => `<div class="skill"><span>${n}</span><div class="sbar"><div style="width:${v}%"></div></div><b>${v}</b></div>`)
    .join('');
}

function popDamage(card: HTMLElement, dmg: number, crit: boolean): void {
...
function showBanner(winner: (FighterActor & { corner?: string }) | null,
  _loser: FighterActor | null, sub: string): void {
  $('#bannerText').textContent = winner ? `${winner.cfg.short} WON!` : 'DRAW';
  const color = winner?.corner ?? '#d4af37';
...
function flashRoundCard(text: string): void {
  const rc = $('#roundCard');
  rc.textContent = text;
...
// TypeScript cannot narrow a module-level `let` inside a callback, because the
// callback could run after a reassignment. Every access below is guarded at
// runtime (ctlActive(), autochessActive, the `if (!engine) return` early exits),
// so these are asserted rather than re-guarded at each of ~90 call sites.
let shake = 0;
const addShake = (s: number): number => (shake = Math.max(shake, s));

// ---------- Fighters / match lifecycle ----------
const fighters: Record<string, Fighter3D> = {};
let fighterA: Fighter3D | null = null;
let fighterB: Fighter3D | null = null;
...
let simPaused = false;
let portraits: Record<string, string> = {}; // face thumbs, reused by roster tiles, corner cards, rankings
const fightStats: Record<string, { sig: number; td: number; ctrl: number; dmg: number; crit: number; stam: number }> = {
...
const fmtCtrl = (sec: number): string => `${Math.floor(sec / 60)}:${String(Math.floor(sec % 60)).padStart(2, '0')}`;
...
let autochessMode: AutochessMode | null = null;
let autochessUi: AutochessUI | null = null;
let autochessActive = false;
let selectedMode = 'auto'; // 'auto' | 'control' | 'autochess'
/** set once loading finishes; every mode entry point checks it first */
let loadedAssets: LoadedAssets | null = null;
/** the octagon GLB, hidden while the autochess cage is up */
let octagonRoot: THREE.Group | null = null;

// ---------- player-control mode (solo only; AUTO-SIM stays the default) ----------
let ctlMode = false;
let ctlStam = 100;
let ctlDodgeCd = 0;
const CTL_COST: Record<string, number> = { light: 16, heavy: 26, special: 34, dodge: 14 };
...
function updateCtlPad() {
  ...
  const ctlButtons: [string, string][] = [['#ctlLight', 'light'], ['#ctlHeavy', 'heavy'], ['#ctlSpec', 'special']];
  for (const [id, slot] of ctlButtons) {
    $<HTMLButtonElement>(id).disabled = !canStrike || ctlStam < CTL_COST[slot];
  }
  $<HTMLButtonElement>('#ctlDodge').disabled = ctlDodgeCd > 0 || ctlStam < CTL_COST['dodge'] || engine?.state !== 'fighting';
}
...
function ctlStrike(slot: 'light' | 'heavy' | 'special'): void {
  if (!ctlActive()) return;
...
function ctlGuard(held: boolean): void {
  if (!ctlActive()) return;
  engine!.playerGuard(held);
  $('#ctlBlock').classList.toggle('held', held);
}

function statRowsHTML(cfg: Fighter): string {
  return [
...
function updateCounters() {
  const sides: [string, string][] = [['a', 'A'], ['b', 'B']];
  for (const [side, suf] of sides) {
    const s = fightStats[side];
    $(`#ccSig${suf}`).textContent = String(s.sig);
    $(`#ccTd${suf}`).textContent = String(s.td);
  ...
  if (!$('#statSheet').classList.contains('hidden')) {
    const cells: [string, (v: number) => string | number][] = [
      ['sig', v => v], ['td', v => v], ['ctrl', fmtCtrl], ['crit', v => v], ['dmg', Math.round]];
    for (const [k, fmt] of cells) {
      for (const side of ['a', 'b']) {
        const cell = document.querySelector(`[data-ss="${side}-${k}"]`);
        const stats = fightStats[side] as unknown as Record<string, number>;
        if (cell) cell.textContent = String(fmt(stats[k]));
      }
    }
  }
}
...
function feedPush(tag: string, text: string, delta = ''): void {
  if (skipping) return;
  const rows = $('#feedRows');
...
function setupBroadcast(): void {
  const surname = (f: Fighter3D): string => (f.cfg.name.split(' ').pop() ?? '').toUpperCase();
  $('#matchTitle').textContent = `${surname(fighterA!)} VS ${surname(fighterB!)}`;
  const corners: [Fighter3D, string, string][] = [[fighterA!, 'A', 'RED'], [fighterB!, 'B', 'GREEN']];
  for (const [f, suf, corner] of corners) {
    ...
    const img = $<HTMLImageElement>(`#ccImg${suf}`);
...
function updateNavTabs(active: string): void {
  document.querySelectorAll<HTMLElement>('.btab').forEach(b => b.classList.toggle('sel', b.dataset['nav'] === active));
}
...
function updateHP() {
  if (!fighterA) return;
  const bars: [Fighter3D, string, string][] =
    [[fighterA!, '#hp-a', '#hpg-a'], [fighterB!, '#hp-b', '#hpg-b']];
  for (const [f, id, gid] of bars) {
    const el = $(id);
    el.style.width = `${f.hp}%`;
    el.style.background = hpColor(f.hp);
    $(gid).style.width = `${f.hp}%`;
    el.parentElement?.setAttribute('aria-valuenow', String(Math.round(f.hp)));
    el.parentElement?.setAttribute('aria-valuetext', `${Math.round(f.hp)} HP`);
  }
  const sides2: [Fighter3D, string][] = [[fighterA!, 'A'], [fighterB!, 'B']];
  for (const [f, suf] of sides2) {
    $(`#sbHp${suf}`).textContent = String(Math.max(0, Math.round(f.hp)));
...
const engineCallbacks: EngineCallbacks = {
  ...
  onDamage: (def, dmg, crit, atk, moveKey) => {
    ...
    const band = document.querySelector<HTMLElement>(def === fighterA ? '.sbA' : '.sbB');
...
function startMatch(idA: string, idB: string, seed: number | null = null): void {
  ...
  (window as unknown as { __fight: unknown }).__fight = { engine, fighters, fighterA, fighterB, simFight };
}
...
function updateMenuPreview(changedSide: string | null = null): void {
  for (const f of Object.values(fighters)) f.root.visible = false;
  for (const side of ['A', 'B']) {
...
function buildMenu(thumbs: Record<string, string>): void {
  ...
  document.querySelectorAll<HTMLElement>('.rtile').forEach(el =>
...
function refreshMenu() {
  ...
  const plates: [string, string, string][] = [['#mplateA', selA, 'A'], ['#mplateB', selB, 'B']];
  for (const [pid, sel, side] of plates) {
    const cfg = FIGHTERS.find(c => c.id === sel)!;
    const p = $(pid);
    p.querySelector<HTMLElement>('.mpname')!.textContent = cfg.name.toUpperCase();
    p.querySelector<HTMLElement>('.mpnick')!.textContent = cfg.nick.toUpperCase();
    p.querySelector<HTMLElement>('.mpstat')!.textContent =
      `STR ${cfg.stats.striking} · GRP ${cfg.stats.grappling} · SPD ${cfg.stats.speed} · CAR ${cfg.stats.cardio} · CHN ${cfg.stats.chin}`;
    p.querySelector<HTMLElement>('.mpready')!.classList.toggle('hidden', !(netConnected() && (side === 'A' ? readyA : readyB)));
...
// actually throw.
const moveDamage = (m: Move): number => m.impacts.reduce((sum, i) => sum + (i.min + i.max) / 2, 0);

function fighterMoveset(cfg: Fighter) {
...
function fighterTraits(cfg: Fighter): string[] {
  const t = [];
...
function renderFighterCard(cfg: Fighter, target: HTMLElement): void {
...
function netToast(message: string): void {
  const el = $('#commentary');
...
/** Lobby/game traffic. Fields are optional because each message type uses a
 *  subset; onNetMsg narrows on `t` before reading them. */
interface NetMessage {
  t?: string; v?: number; side?: string; id?: string; pick?: string;
  ready?: boolean; seed?: number; a?: string; b?: string; mode?: string;
  round?: number; board?: unknown;
}

const netCallbacks: NetCallbacks = {
  onOpen: () => updateNetUi(),
  onConnect: () => {
    readyA = readyB = false;
  ...
  onMsg: d => onNetMsg(d as NetMessage),
  onClose: () => {
    readyA = readyB = false;
    if (net?.isHost) {
...
function onNetMsg(d: NetMessage): void {
  // autochess round traffic is handled by the mode's own state machine
  if (typeof d.t === 'string' && d.t.startsWith('ac-')) {
    autochessMode?.netMatch?.onMessage(d as { t: string; round?: number; board?: never });
    return;
  }
  switch (d.t) {
    case 'hello':
    case 'pick': {
      ...
      const id = (d.pick ?? d.id) as string;
    ...
    case 'ready':
      ...
      if (d.side === 'A') readyA = d.ready ?? false; else readyB = d.ready ?? false;
      refreshMenu();
      updateNetUi();
      maybeHostStart();
      break;
    case 'start':
      ...
      selA = d.a as string;
      selB = d.b as string;
      startMatch(selA, selB, d.seed ?? null);
      break;
    case 'rematch':
      if (typeof d.seed !== 'number') { teardownNet('GAME VERSIONS DIFFER — RELOAD THE PAGE ON BOTH PHONES'); break; }
      doRematch(d.seed);
...
function teardownNet(message: string | null = null): void {
  net?.destroy();
  net = null;
...
function updateNetUi() {
  ...
  $<HTMLButtonElement>('.modeBtn[data-mode="control"]').disabled = !!net;
  const fight = $<HTMLButtonElement>('#menuFightBtn');
  if (net) {
    $('#netStatus').innerHTML = netConnected()
      ...
      : net!.isHost
        ...
        : 'CONNECTING…';
    const mic = $<HTMLButtonElement>('#netMicBtn');
  ...
  const gmic = $<HTMLButtonElement>('#gameMicBtn');
  if (net && !menuOpen) {
...
function doRematch(seed: number | null): void {
...
$<HTMLButtonElement>('#netHostBtn').addEventListener('click', () => {
  fx.init();
  unlockRemoteAudio();
  teardownNet();
...
$<HTMLButtonElement>('#netJoinBtn').addEventListener('click', () => {
  fx.init();
  unlockRemoteAudio();
  const code = $<HTMLInputElement>('#netCodeInput').value.trim();
  if (!/^\d{4}$/.test(code)) {
...
$<HTMLInputElement>('#netCodeInput').addEventListener('keydown', e => {
  if (e.key === 'Enter') $<HTMLButtonElement>('#netJoinBtn').click();
});

const toggleMic = () => {
  if (net && net!.micState !== 'blocked' && net!.micState !== 'off') net!.toggleMute();
};
$<HTMLButtonElement>('#netMicBtn').addEventListener('click', toggleMic);
$<HTMLButtonElement>('#gameMicBtn').addEventListener('click', toggleMic);

$<HTMLButtonElement>('#netLeaveBtn').addEventListener('click', () => teardownNet('LEFT THE LOBBY'));

$<HTMLButtonElement>('#menuFightBtn').addEventListener('click', () => {
  fx.init();
  if (net) {
...
$<HTMLButtonElement>('#rematchBtn').addEventListener('click', () => {
  if (net) {
    if (!netConnected()) return;
    if (net!.isHost) {
      const seed = (Math.random() * 0x7fffffff) | 0;
...
$<HTMLButtonElement>('#changeBtn').addEventListener('click', () => {
  if (net && netConnected()) net!.send({ t: 'menu' });
  closeBannerToMenu();
});

// ---------- broadcast controls ----------
$<HTMLButtonElement>('#pauseBtn').addEventListener('click', () => {
  simPaused = !simPaused;
  $('#pauseLabel').textContent = simPaused ? 'Resume sim' : 'Pause sim';
});

document.querySelectorAll<HTMLElement>('.speedBtn').forEach(b =>
  b.addEventListener('click', () => {
    simSpeed = Number(b.dataset['speed']) || 1;
    document.querySelectorAll<HTMLElement>('.speedBtn').forEach(x => x.classList.toggle('sel', x === b));
  }),
);

$<HTMLButtonElement>('#skipBtn').addEventListener('click', () => skipToResult());
...
document.querySelectorAll<HTMLElement>('.modeBtn').forEach(b =>
  b.addEventListener('click', () => {
    selectedMode = b.dataset['mode'] ?? 'auto';
    ctlMode = selectedMode === 'control';
    document.querySelectorAll<HTMLElement>('.modeBtn').forEach(x => x.classList.toggle('sel', x === b));
    $('#menuHint').textContent = MODE_HINTS[selectedMode] || HINT_DEFAULT;
    $<HTMLButtonElement>('#menuFightBtn').textContent = selectedMode === 'autochess' ? 'ENTER THE CAGE' : 'FIGHT';
  }),
);

// ---------- autochess entry / exit ----------
async function enterAutochess(online: { isHost: boolean; seed: number } | null = null): Promise<void> {
  ...
  if (!autochessMode) {
    ...
    try {
      ...
      console.error(err);
      loadText.textContent = `Board failed to load — ${err instanceof Error ? err.message : String(err)}`;
      autochessMode = null;
      return;
    }
  ...
  if (online) {
    const nm = autochessMode!.netMatch!;
    if (online.isHost) nm.start(online.seed);
    else { nm.seed = online.seed | 0; nm.started = true; nm.round = 1; }
  }
  (window as unknown as { __autochess: unknown }).__autochess = autochessMode;
}
...
document.addEventListener('keydown', e => {
  if (!ctlActive() || e.repeat || (e.target as HTMLElement | null)?.tagName === 'INPUT') return;
...
$<HTMLButtonElement>('#statsBtn').addEventListener('click', () => {
...
document.addEventListener('click', e => {
  const close = (e.target as HTMLElement | null)?.dataset?.['close'];
  if (close) {
...
document.querySelectorAll<HTMLElement>('.btab').forEach(b =>
  b.addEventListener('click', () => {
    ...
    if (nav === 'fight') {
      ...
      if (menuOpen) {
        if (!$<HTMLButtonElement>('#menuFightBtn').disabled) $<HTMLButtonElement>('#menuFightBtn').click();
...
loadAssets(FIGHTERS, (loaded, total) => {
  ...
  loadedAssets = { models, clipSets, overlayDeltas, octagon };
  octagonRoot = octagon;
  ...
  openMenu();
  (window as unknown as { __fight: unknown }).__fight = { fighters, simFight };
}).catch(err => {
  console.error(err);
...
let frameEMA = 16;
let lastDownshift = 0;
function autoQuality(rawDtMs: number, t: number): void {
...
function autochessCellFromEvent(e: PointerEvent) {
