/** Minimal surface of the vendored PeerJS UMD bundle (loaded via a script tag,
 *  so it arrives on `window` rather than through the module graph). */
interface PeerConnection {
  ...
  on(ev: 'error', cb: (e: PeerError) => void): void;
}
interface MediaConnection {
  answer(stream?: MediaStream): void;
  close(): void;
  on(ev: 'stream', cb: (s: MediaStream) => void): void;
}
export interface PeerError { type: string; message?: string; }
interface PeerLike {
  destroyed: boolean;
  disconnected: boolean;
  destroy(): void;
  reconnect(): void;
  connect(id: string, opts?: { reliable?: boolean }): PeerConnection;
  call(id: string, stream: MediaStream): MediaConnection;
  on(ev: 'open', cb: () => void): void;
  on(ev: 'connection', cb: (c: PeerConnection) => void): void;
  on(ev: 'call', cb: (c: MediaConnection) => void): void;
  on(ev: 'error', cb: (e: PeerError) => void): void;
  on(ev: 'disconnected', cb: () => void): void;
}
declare const Peer: new (id?: string | object, opts?: object) => PeerLike;

export type MicState = 'off' | 'on' | 'muted' | 'blocked';
...
export interface NetCallbacks {
  ...
  onRetry?(n: number, max: number): void;
...
export const PROTOCOL_VERSION = 7; // v7: Karina and Lika join the roster (FIGHTERS order, unit ids)
...
export class Net {
  ...
  private peer: PeerLike | null;
  /** read by netConnected(); the lobby UI gates on an open channel */
  conn: PeerConnection | null;
  private mediaCall: MediaConnection | null;
  private micStream: MediaStream | null;
  isHost: boolean;
  code: string | null;
  /** peer 'open' confirmed by the signaling server */
  registered: boolean;
  micState: MicState;
  private _closed: boolean;
  private _tries: number;
  private _reconnects: number;
  private _reconnT: ReturnType<typeof setTimeout> | null = null;
  private _retryT: ReturnType<typeof setTimeout> | null = null;
  ...
  constructor(cb: NetCallbacks) {
    ...
    this._visHandler = () => {
      if (document.visibilityState === 'visible' && this.peer && !this.peer.destroyed && this.peer.disconnected) {
        clearTimeout(this._reconnT ?? undefined);
        this._reconnT = null;
  ...
  private _handlePeerError(err: PeerError, restart: () => void): void {
  ...
  private _attach(conn: PeerConnection): void {
    this.conn = conn;
    conn.on('open', () => {
      clearTimeout(this._watchdog ?? undefined);
      this._watchdog = null;
      this._reconnects = 0;
      this.cb.onConnect();
    });
  ...
  private async _ensureMic(): Promise<void> {
  ...
  private _silentStream(): MediaStream | null {
    try {
  ...
  private _error(err: PeerError): void {
  ...
  destroy(): void {
    this._closed = true;
    clearTimeout(this._watchdog ?? undefined);
    clearTimeout(this._retryT ?? undefined);
    clearTimeout(this._reconnT ?? undefined);
    ...
    try { this.peer?.destroy(); } catch { /* ignore */ }
    this.micStream = null; this.mediaCall = null; this.conn = null; this.peer = null;
  }
}
