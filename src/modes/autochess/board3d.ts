...
/** One painted grid cell. `side` flips for the online guest, whose screen is
 *  rotated rather than its coordinates mirrored. */
interface GridCell {
  ring: THREE.Mesh;
  fill: THREE.Mesh;
  col: number;
...
export type Highlight = 'hover' | 'valid' | 'occupied';
...
function hexRingGeometry(radius: number, thickness: number): THREE.ShapeGeometry {
...
export class Board3D {
  ...
  /** units parent here so their local y=0 sits on the cage floor */
  readonly unitRoot: THREE.Group;
  readonly gridRoot: THREE.Group;
  readonly cells: Map<number, GridCell>;
  /** global grid opacity multiplier: 1 while planning, low during combat */
  dim: number;
  private _state: Map<number, Highlight> | null;
  floorY: number;
  loaded: boolean;
  flipped = false;
  cage?: THREE.Group;
  ...
  private readonly _hit: THREE.Vector3;
  ...
  async load(onProgress?: (loaded: number, total: number) => void): Promise<void> {
    ...
    const gltf = await loader.loadAsync('assets/arena/autochess_board.glb');
    ...
    cage.traverse((o: THREE.Object3D) => {
  ...
  setHighlights(state: Map<number, Highlight> | null): void {
    this._state = state;
    const d = this.dim;
    for (const [id, cell] of this.cells) {
      ...
      if (s === 'hover') {
        (cell.ring.material as THREE.MeshBasicMaterial).color.setHex(HEX_COLORS.hover);
        (cell.ring.material as THREE.MeshBasicMaterial).opacity = 0.95 * d;
        (cell.fill.material as THREE.MeshBasicMaterial).color.setHex(HEX_COLORS.hover);
        (cell.fill.material as THREE.MeshBasicMaterial).opacity = 0.3 * d;
      } else if (s === 'valid') {
        (cell.ring.material as THREE.MeshBasicMaterial).color.setHex(HEX_COLORS.valid);
        (cell.ring.material as THREE.MeshBasicMaterial).opacity = 0.7 * d;
        (cell.fill.material as THREE.MeshBasicMaterial).color.setHex(HEX_COLORS.valid);
        (cell.fill.material as THREE.MeshBasicMaterial).opacity = 0.13 * d;
      } else if (s === 'occupied') {
        (cell.ring.material as THREE.MeshBasicMaterial).color.setHex(HEX_COLORS.occupied);
        (cell.ring.material as THREE.MeshBasicMaterial).opacity = 0.55 * d;
        (cell.fill.material as THREE.MeshBasicMaterial).opacity = 0;
      } else {
        (cell.ring.material as THREE.MeshBasicMaterial).color.setHex(base);
        (cell.ring.material as THREE.MeshBasicMaterial).opacity = 0.34 * d;
        (cell.fill.material as THREE.MeshBasicMaterial).opacity = 0;
      }
    }
  }
  ...
  setGridMode(planning: boolean): void {
  ...
  cellAtPointer(ndc: THREE.Vector2, camera: THREE.Camera): Hex.Cell | null {
  ...
  worldOf(col: number, row: number, out = new THREE.Vector3()): THREE.Vector3 {
    const w = Hex.cellToWorld(col, row);
