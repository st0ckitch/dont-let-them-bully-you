...
export const cellId = (col, row) => row * COLS + col;
export const idCol = id => id % COLS;
export const idRow = id => Math.floor(id / COLS);
export const inBounds = (col, row) => col >= 0 && col < COLS && row >= 0 && row < ROWS;
export const isPlayerHalf = row => row >= HALF;
...
export function cellToWorld(col, row, out = { x: 0, z: 0 }) {
...
export function stepToward(from, goal, blocked) {
  ...
  const w = { x: 0, z: 0 };
  let best = null, bestD = Infinity, bestE = Infinity;
