...
// here rather than hand-authored, so a balance change in fighters/ still reads
...
import type { Ability, Unit, StarStats } from './types.ts';
export type { Ability, Unit, StarStats, Entry, BoardSpec, UnitBase, AbilityKind } from './types.ts';
...
const TIER_HP: Record<number, number> = { 1: 485, 2: 595, 3: 730, 4: 910, 5: 1160 };
const TIER_AD: Record<number, number> = { 1: 48, 2: 58, 3: 70, 4: 88, 5: 114 };
const TIER_RESIST: Record<number, number> = { 1: 20, 2: 26, 3: 33, 4: 45, 5: 60 };

// star scaling — TFT's evergreen rule: each star multiplies HP and AD by 1.8
export const STAR_SCALE: number[] = [0, 1, 1.8, 3.24];

const lerp = (a: number, b: number, t: number): number => a + (b - a) * t;
// map a 0-100 fighter stat onto a multiplier band centred on 1.0
const band = (v: number, spread: number): number => lerp(1 - spread, 1 + spread, Math.max(0, Math.min(1, (v - 78) / 22)));
...
export function abilityText(unit: Unit, star = 1): string {
...
export const ATTACK_IMPACT_AT: Record<string, number> = {
...
const RANGE_TAX: Record<number, { hp: number; ad: number }> = { 1: { hp: 1, ad: 1 }, 2: { hp: 0.9, ad: 0.95 } };
...
const TUNE: Record<string, { hp: number; ad: number; as: number }> = {
...
export function statsFor(unit: Unit, star: number): StarStats {
...
export const UNITS_BY_COST = (() => {
  const m: Record<number, Unit[]> = { 1: [], 2: [], 3: [], 4: [], 5: [] };
