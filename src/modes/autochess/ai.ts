...
import { UNIT_BY_ID, sellValue, statsFor } from './units.ts';
import { Roster, Economy, SHOP_SIZE, REROLL_COST, XP_COST, XP_PER_ROUND, boardCapacity } from './shop.ts';
import type { Pool } from './shop.ts';
import * as Hex from './hex.ts';
import type { Entry, AiBoardSpec } from './types.ts';

type Rng = () => number;

/** A named comp the AI commits to at game start, so its board reads as a
 *  strategy rather than a drawer of parts. */
interface Comp { name: string; carry: string; core: string[]; }

export interface AiTuning {
  levelTempo: (round: number) => number;
  reserve: (round: number, hp: number) => number;
  maxRolls: (round: number, hp: number, gold: number) => number;
  rollThreshold: number;
}

export interface AiSnapshot {
  level: number; gold: number; owned: string[]; board: string[];
}

let rng: Rng = Math.random;
export function setAiRng(fn: Rng | null): void { rng = fn || Math.random; }
...
export const AI_TUNING: AiTuning = {
  ...
  levelTempo: (r: number) => (r <= 2 ? 3 : r <= 4 ? 4 : r <= 7 ? 5 : r <= 10 ? 6 : r <= 14 ? 7 : r <= 18 ? 8 : 9),
  ...
  reserve: (r: number, hp: number) => (hp <= 50 ? 0 : r <= 3 ? 0 : r <= 8 ? 10 : 20),
  ...
  maxRolls: (r: number, hp: number, gold: number) => {
    let n = r <= 3 ? 1 : r <= 8 ? 3 : 5;
...
function wantScore(ai: AiOpponent, unitId: string): number {
...
// on the bench.
const fieldRank = (e: Entry): number => e.star * 1000 + UNIT_BY_ID[e.unitId].cost * 10;
...
export class AiOpponent {
  ...
  takeTurn(roundIndex: number): AiBoardSpec[] {
    const r = roundIndex;
  ...
  private _levelUp(r: number): void {
  ...
  private _shopPhase(r: number): void {
    const reserve = AI_TUNING.reserve(r, this.econ.hp);
  ...
  private _buyFrom(shop: (string | null)[], r: number): boolean {
    const reserve = AI_TUNING.reserve(r, this.econ.hp);
    const ranked = shop
      .map((id, i) => (id ? { id, i, score: wantScore(this, id) } : null))
      .filter((c): c is { id: string; i: number; score: number } => c !== null)
  ...
  private _makeBenchRoom(): boolean {
  ...
  buildBoard(): AiBoardSpec[] {
    ...
    const specs: AiBoardSpec[] = [];
    const put = (x: { e: Entry }, slot: Hex.Cell | undefined): boolean => {
      if (!slot) return false;
      x.e.cell = Hex.cellId(slot.col, slot.row);
      specs.push({ id: x.e.unitId, star: x.e.star, col: slot.col, row: slot.row });
      return true;
    };
    const anywhere = (): Hex.Cell | undefined => front.shift() || back.shift();
...
const mirror = (col: number, row: number): Hex.Cell => ({ col: Hex.COLS - 1 - col, row: Hex.ROWS - 1 - row });
