...
import { UNITS, UNITS_BY_COST, UNIT_BY_ID, POOL_COPIES } from './units.ts';
import type { Entry } from './types.ts';

export interface Payout { base: number; interest: number; streak: number; total: number; }
export interface MergeResult { entry: Entry | null; merged: { entry: Entry; star: number }[]; }
export interface Progress { star: number; have: number; need: number; }

type Rng = () => number;
let rng: Rng = Math.random;
export function setShopRng(fn: Rng | null): void { rng = fn || Math.random; }
...
export const XP_TO_NEXT: Record<number, number> = { 1: 2, 2: 2, 3: 6, 4: 10, 5: 18, 6: 26, 7: 34, 8: 48 };

// Board slots equal player level, exactly like TFT.
export const boardCapacity = (level: number): number => level;
...
export class Pool {
  ...
  countOf(id: string): number { return this.counts[id] || 0; }
  ...
  roll(level: number, n = SHOP_SIZE): (string | null)[] {
...
let nextEntryUid = 1;
export const makeEntry = (unitId: string, star = 1): Entry => ({ uid: nextEntryUid++, unitId, star, cell: null });
...
export class Roster {
  ...
  get bench(): Entry[] { return this.entries.filter(e => e.cell === null); }
  get board(): Entry[] { return this.entries.filter(e => e.cell !== null); }
  benchFull(): boolean { return this.bench.length >= BENCH_SLOTS; }
  at(cell: number): Entry | null { return this.entries.find(e => e.cell === cell) || null; }
  ...
  add(unitId: string, star = 1): MergeResult {
    const e = makeEntry(unitId, star);
    this.entries.push(e);
    const merged: { entry: Entry; star: number }[] = [];
  ...
  private _resolveMerges(unitId: string, merged: { entry: Entry; star: number }[]): void {
  ...
  copiesToward(unitId: string): Progress {
...
export class Economy {
  ...
  canAfford(n: number): boolean { return this.gold >= n; }
  ...
  xpToNext(): number { return this.level >= MAX_LEVEL ? 0 : XP_TO_NEXT[this.level]; }
