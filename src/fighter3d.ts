...
import type { FighterActor } from './modes/octagon/fight.ts';
import type { PreparedModel } from './anim.ts';

export interface Fighter3DOpts {
  model: PreparedModel;
  clips: Record<string, THREE.AnimationClip>;
  pos: THREE.Vector3;
  corner: string;
  /** per-rig bone-frame correction for the additive overlays; null on athletic */
  bindDelta?: Record<string, THREE.Quaternion> | null;
  /** autochess parents units to a board group instead of the scene */
  parent?: THREE.Object3D | null;
  /** the hex grid is far wider than the octagon, so autochess passes Infinity */
  arenaRadius?: number;
}
...
const GUARD_DELTAS: [string, [number, number, number]][] = [
  ['LeftArm', [-0.12, 0, 0.15]],
...
export class Fighter3D implements FighterActor {
  readonly cfg: Fighter;
  readonly stats: Fighter['stats'];
  hp: number;
  corner: string;
  ...
  readonly bindDelta: Record<string, THREE.Quaternion> | null;
  guardTarget = 0;
  guardW = 0;
  pulseT = 0;
  evadeT = 0;
  ...
  flashI = 0;
  flinchI = 0;
  flinchAxes: Record<string, THREE.Vector3> = {};
  koT = 0;
  yaw = 0;
  yawTarget = 0;
  ...
  constructor(cfg: Fighter, scene: THREE.Object3D,
    { model, clips, pos, corner, bindDelta = null, parent = null, arenaRadius = 2.6 }: Fighter3DOpts) {
    this.cfg = cfg;
    ...
    const deltaAxis = (name: string): THREE.Vector3 => {
  ...
  // ---- animation ----
  play(key: string, { once = false, fade = 0.22, timeScale = 1, onDone = null }: PlayOpts = {}): void {
  ...
  // ---- movement ----
  /** live position; the sim reads and writes this directly */
  ...
  faceToward(point: { x: number; y: number; z: number }, dt: number, instant = false): void {
  ...
  moveToward(point: { x: number; y: number; z: number }, speed: number, dt: number): number {
  ...
  update(dt: number): void {
    ...
    if (gw > 0.01 && this.state !== 'ko' && this.currentKey !== null && GUARD_SAFE.has(this.currentKey)) {
    ...
    for (const m of this.materials) (m as THREE.MeshStandardMaterial).emissiveIntensity = this.flashI * 0.5;
  }
}
